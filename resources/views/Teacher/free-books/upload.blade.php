...
<head>
    ...
    <link href="{{ asset("assets/teacher_assets/assets/css/bootstrap.css") }}" rel="stylesheet">
            ...
            <div class="col-md-12">
                <div class="card w-50 m-auto">
                    <div class="card-header bg-info text-white">
                        <h4>رفع الكتاب ل  {{ $lesson->title }}</h4>
                    </div>
                    <div class="card-body">
                    ...
                    <div style="margin: 20px;">
                        <a href="{{ route("free-books") }}" class="btn btn-outline-info" id="main">العودة للصفحة الرئيسية</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        $(document).ready(function () {
            ...
            init: {
                ...
                FileUploaded: function(up, file, result){
                    ...
                    setTimeout(() => {
                        location.href="{{ route("free-books") }}"
                    }, 3000);

                    if (responseResult.ok==0) {
                        toastr.error(responseResult.info, 'Error Alert', {timeOut: 2000});

                    }
                    if (result.status != 200) {
                        toastr.error('عفوا لم يتم رفع الكتاب', 'Error Alert', {timeOut: 2000});
                    }
                    if (responseResult.ok==1 && result.status == 200) {
                        toastr.success('تم رفع الكتاب بنجاح', 'رفع الكتاب', {timeOut: 2000});
                    }
                },
